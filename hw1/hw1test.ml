...
let my_subset_test0 = not (subset [3;2;1;9] [])
let my_subset_test1 = subset [] []

let my_equal_sets_test0 = not (equal_sets [1;3;4;5;3;1;5;6] [4;4;3;1;3;5])

let my_set_union_test0 = not (equal_sets (set_union [] []) [1])
let my_set_union_test1 = equal_sets (set_union [3] [1;2;3]) [1;2;3]

let my_set_intersection_test0 =
  equal_sets (set_intersection [4] [1;2;3]) []

let my_set_diff_test0 = equal_sets (set_diff [1;3;3;1] [1;4;3;1]) []

let my_computed_fixed_point_test0 =
  computed_fixed_point (=) sqrt(sqrt 9.) = 1. 

let my_computed_periodic_point_test0 =
  computed_periodic_point (=) (fun x -> x / 2) 1 1 = 0

let my_while_away_test0 = while_away ((+) 4) ((>) 12) 9 = [9]

let my_rle_decode_test0 = rle_decode [2,"e"; 2,"g"; 1,"r"; 0,"t"] = ["e"; "e"; "g"; "g"; "r"]

type my_nonterminals =
  | Expr | Atom | Symbol | Num | Loop

let my_rules =
   [Expr, [T"("; N Expr; T")"];
    Expr, [N Num];
    Expr, [N Expr; N Atom; N Expr];
    Expr, [N Symbol];
    Expr, [N Atom; N Symbol];
    Atom, [T"^^"; N Expr];
    Atom, [T"++"];
    Atom, [T"--"];
    Symbol, [N Expr; N Loop];
    Num, [T"0"];
    Num, [T"1"];
    Num, [T"2"];
    Num, [T"3"];
...
let my_test0 =
  ...
  (Expr,
   [(Expr, [T "("; N Expr; T ")"]); (Expr, [N Num]);
    (Expr, [N Expr; N Atom; N Expr]); (Atom, [T "^^"; N Expr]);
    (Atom, [T "++"]); (Atom, [T "--"]); (Num, [T "0"]); (Num, [T "1"]);
