...
let my_accept_all my_derivation string = Some (my_derivation, string)
let my_accept_empty_suffix my_derivation = function
   | [] -> Some (my_derivation, [])
   | _ -> None
...
let test_1 =
  ((parse_prefix my_grammar my_accept_empty_suffix ["9";"++"]) 
    = None)

let test_2 =
  ((parse_prefix my_grammar accept_all ["(";"9";"++";")";"+";"(";"1";"--";")"])
   = Some
    ([(Expr, [N Term; N Binop; N Expr]);
    (Term, [T "("; N Num; N Incrop; T ")"]); (Num, [T "9"]);
    (Incrop, [T "++"]); (Binop, [T "+"]); (Expr, [N Term]);
    (Term, [T "("; N Num; N Incrop; T ")"]); (Num, [T "1"]);
    (Incrop, [T "--"])], []))
